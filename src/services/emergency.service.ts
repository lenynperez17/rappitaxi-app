/**
 * SERVICIO DE EMERGENCIAS RAPPI TEAM
 * ====================================
 * 
...
class EmergencyService {
  ...
  // Administradores de Rappi Team
  private readonly ADMIN_PHONES = [
    '+51946123456', // Central Rappi Team
  ...
  async triggerSOS(
    ...
    try {
      ...
      // 6. ALERTAR A ADMINISTRADORES DE RAPPI TEAM
  ...
  async shareLocationWithContacts(
    ...
    for (const contact of contacts) {
      try {
        const message = `🚨 EMERGENCIA RAPPI TEAM 🚨
...
Tu contacto de emergencia ha cancelado la alerta de emergencia de Rappi Team.
...
- Central Rappi Team`;
  ...
  private async alertAdministrators(emergency: EmergencyAlert): Promise<void> {
    const message = `🚨 EMERGENCIA ACTIVA - RAPPI TEAM 🚨
...
RESPONDER INMEDIATAMENTE - Panel admin: https://admin.rapi-team.com/emergencies/${emergency.id}`;
