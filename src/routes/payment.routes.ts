...
const WEBHOOK_URL = process.env.WEBHOOK_URL || 'https://api.rapiteam.app/api/v1/payments/webhook';
  ...
  try {
    ...
    const preferenceData = {
      items: [
        {
          id: rideId,
          title: description || `Viaje RapiTeam #${rideId}`,
          quantity: 1,
...
router.post('/process-yape', [
  ...
  try {
    ...
    const paymentData = {
      ...
      createdAt: Timestamp.now(),
      metadata: {
        qrCode: `yape://payment?amount=${amount}&phone=946123456`, // Número de RapiTeam
    ...
    const yapeQRData = {
      phoneNumber: '946123456', // Número de RapiTeam para recibir pagos
      amount,
...
router.post('/process-plin', [
  ...
  try {
    ...
    const plinQRData = {
      phoneNumber: '946123456', // Número de RapiTeam
      amount,
      message: `Viaje #${rideId}`,
