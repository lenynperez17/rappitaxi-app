...
class PriceNegotiationProvider extends ChangeNotifier {
  ...
  /// Verificar si el conductor tiene créditos de servicio suficientes para operar
  Future<bool> checkDriverBalance(String driverId) async {
    try {
      final walletDoc = await _firestore.collection('wallets').doc(driverId).get();
      if (!walletDoc.exists) {
        debugPrint('Conductor sin billetera: $driverId');
        return false;
      }

      final walletData = walletDoc.data()!;
      // Usar serviceCredits (créditos de servicio), NO balance (ganancias)
      final serviceCredits = (walletData['serviceCredits'] ?? 0.0).toDouble();

      debugPrint('Créditos conductor $driverId: S/. $serviceCredits (mínimo: S/. ${CreditConstants.minServiceCredits})');
      return serviceCredits >= CreditConstants.minServiceCredits;
    } catch (e) {
      debugPrint('Error verificando créditos: $e');
      return false;
    }
  }

  /// Getter para obtener el saldo mínimo requerido
  double get minimumDriverBalance => CreditConstants.minServiceCredits;
  ...
  Future<String?> makeDriverOffer(String negotiationId, double acceptedPrice) async {
    try {
      ...
      // Verificar créditos de servicio mínimos antes de hacer oferta
      final hasBalance = await checkDriverBalance(user.uid);
      if (!hasBalance) {
        debugPrint('Conductor sin créditos suficientes para hacer ofertas');
        return 'Créditos insuficientes. Necesitas mínimo S/. ${CreditConstants.minServiceCredits.toStringAsFixed(2)} para hacer ofertas. Recarga tus créditos.';
      }
      ...
      return null; // Negociacion no encontrada localmente pero no es error
    } catch (e) {
      debugPrint('Error haciendo oferta: $e');
      return FirestoreErrorHandler.getSpanishMessage(e);
    }
  }
