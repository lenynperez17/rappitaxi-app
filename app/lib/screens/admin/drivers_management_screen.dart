...
class _DriversManagementScreenState extends State<DriversManagementScreen>
    ...
    return Scaffold(
      ...
      body: _isLoading
        ...
        children: [
                    ...
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        colors: [
                          ...
                          ModernTheme.rappiOrange.withBlue(30),
                        ],
                      ),
                    ),
                    child: Row(
            ...
            child: Padding(
              ...
              children: [
                  ...
                  child: Container(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: 'Buscar por nombre, email, placa...',
                ...
                PopupMenuButton<String>(
                  icon: Icon(Icons.sort, color: ModernTheme.rappiOrange),
                  onSelected: (value) {
          ...
          // Tabs de estado (dentro del mismo Material sticky)
          Material(
            elevation: 1,
        ...
        child: Padding(
          ...
          child: Column(
            children: [
                ...
                children: [
                  Stack(
                    children: [
                      CircleAvatar(
                        radius: 30,
                        backgroundImage: (driver.photo.isNotEmpty && driver.photo.startsWith('http'))
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(
                                color: _getStatusColor(driver.status).withValues(alpha: 0.4),
                                blurRadius: 4,
                                spreadRadius: 1,
                              ),
                            ],
              ...
              // Estadísticas - envuelto para evitar overflow
              Wrap(
              ...
              // Acciones - usando Wrap para evitar overflow
              Wrap(
                ...
                children: [
                  if (driver.status == DriverStatus.pending) ...[
                    TextButton.icon(
                      ...
                      icon: Icon(Icons.close, color: ModernTheme.error, size: 18),
                      label: Text('Rechazar', style: TextStyle(color: ModernTheme.error, fontSize: 12)),
                    ),
                    ElevatedButton.icon(
                      onPressed: () => _approveDriver(driver),
                      icon: Icon(Icons.check, color: Theme.of(context).colorScheme.onPrimary, size: 18),
                      label: Text('Aprobar', style: TextStyle(fontSize: 12)),
                    ...
                    TextButton.icon(
                      ...
                      icon: Icon(Icons.block, color: ModernTheme.warning, size: 18),
                      label: Text('Suspender', style: TextStyle(color: ModernTheme.warning, fontSize: 12)),
                    ),
                  ] else if (driver.status == DriverStatus.suspended) ...[
                    ElevatedButton.icon(
                      onPressed: () => _activateDriver(driver),
                      icon: Icon(Icons.check_circle, color: Theme.of(context).colorScheme.onPrimary, size: 18),
                      label: Text('Activar', style: TextStyle(fontSize: 12)),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: ModernTheme.rappiOrange,
                        padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                      ),
  ...
  Widget _buildMiniStat(IconData icon, String value, String label, Color color) {
    // ✅ FIX: Removido Flexible para usar con Wrap
    return Column(
  ...
  Widget _buildDocumentsRow(List<Document> documents) {
    // ✅ FIX: Cambiar Row a Wrap para evitar overflow con muchos documentos
    return Wrap(
  ...
  void _sendMessage(Driver driver) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            Icon(Icons.message, color: ModernTheme.rappiOrange),
            SizedBox(width: 12),
            Text('Enviar Mensaje'),
          ],
        ),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              // ✅ SEGURIDAD: Validación de contraseña fuerte unificada
              final strongPasswordPattern = RegExp(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*(),.?":{}|<>]).{8,}$');
              if (!strongPasswordPattern.hasMatch(password)) {
                messenger.showSnackBar(
                  SnackBar(
                    content: Text('La contraseña debe tener mínimo 8 caracteres, mayúscula, minúscula, número y carácter especial (!@#\$%^&*)'),
                    backgroundColor: ModernTheme.error,
                  ),
                );
                return;
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          title: Row(
            children: [
              Icon(Icons.filter_list, color: ModernTheme.rappiOrange),
              SizedBox(width: 12),
  ...
  Future<void> _exportData() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(file.path)],
        text: 'Listado de Conductores - Rappi Team',
      );
