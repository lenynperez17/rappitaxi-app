...
class _FinancialScreenState extends State<FinancialScreen>
  ...
  Future<void> _loadFinancialData() async {
    ...
    try {
      ...
      for (var tripDoc in tripsSnapshot.docs) {
        ...
        final fare = ((tripData['finalFare'] ?? tripData['estimatedFare'] ?? 0.0) as num).toDouble();
      ...
      for (var withdrawalDoc in pendingWithdrawalsSnapshot.docs) {
        ...
        final amount = ((withdrawalData['amount'] ?? 0.0) as num).toDouble();
  ...
  Future<void> _loadCommissionSettings() async {
    try {
      ...
      if (doc.exists) {
        final data = doc.data()!;
        setState(() {
          _currentCommissionRate = ((data['defaultRate'] ?? 20.0) as num).toDouble();
          _commissionRates = {
            'standard': ((data['standard'] ?? 18.0) as num).toDouble(),
            'premium': ((data['premium'] ?? 22.0) as num).toDouble(),
            'corporate': ((data['corporate'] ?? 15.0) as num).toDouble(),
          };
        });
      }
    } catch (e) {
  ...
  Widget _buildGeneralTab() {
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          // Large main chart (approx 45% of screen height)
          Container(
                    ...
                    builder: (context, child) {
                      return CustomPaint(
                        ...
                        painter: RevenueChartPainter(
                          ...
                          data: [1200, 1500, 1300, 1800, 2100, 1900, 2300],
                        ),
                      );
                    },
                  ),
  ...
  // Horizontal KPIs strip (replaces old grid in general tab)
  Widget _buildHorizontalKPIs() {
    final kpis = [
      {'title': 'Ingresos', 'value': _financialStats['totalRevenue']!.toCurrency(), 'icon': Icons.trending_up, 'color': ModernTheme.success},
      {'title': 'Comisiones', 'value': _financialStats['totalCommissions']!.toCurrency(), 'icon': Icons.percent, 'color': ModernTheme.primaryBlue},
      {'title': 'Pagos Pend.', 'value': _financialStats['pendingPayouts']!.toCurrency(), 'icon': Icons.schedule, 'color': ModernTheme.warning},
      {'title': 'Prom. Diario', 'value': _financialStats['dailyAverage']!.toCurrency(), 'icon': Icons.calendar_today, 'color': ModernTheme.rappiOrange},
    ];
    return SizedBox(
      height: 90,
        ...
        Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              prefixIcon: Icon(Icons.search, color: ModernTheme.rappiOrange),
      ...
      children: periods.map((p) {
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isSelected ? ModernTheme.rappiOrange : ModernTheme.rappiOrange.withValues(alpha: 0.08),
              borderRadius: BorderRadius.circular(14),
            ),
            child: Text(
              p['label']!,
              style: TextStyle(
                color: isSelected ? Colors.white : ModernTheme.rappiOrange,
                fontSize: 11,
            ...
            Row(
              children: [
                CircleAvatar(
                  ...
                  child: Icon(Icons.person, color: ModernTheme.rappiOrange),
                ),
                SizedBox(width: 12),
                Expanded(
                  child: Column(
                ...
                Text(
                  (payout['amount'] as num).toDouble().toCurrency(),
                  style: TextStyle(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'Pago aprobado: ${(payout['amount'] as num).toDouble().toCurrency()} a ${payout['driverName']}'),
            backgroundColor: ModernTheme.success,
          ),
        );
      }
  ...
  Future<void> _exportFinancialReport() async {
    try {
          ...
          build: (context) => [
            ...
            pw.Header(
              ...
              child: pw.Column(
                ...
                children: [
                  pw.Text('REPORTE FINANCIERO - RAPPI TEAM',
      ...
      await Share.shareXFiles(
        ...
        text: 'Reporte Financiero - Rappi Team',
      );
        ...
        pw.Page(
          build: (context) => pw.Column(
            ...
            children: [
              ...
              pw.Divider(thickness: 2),
              pw.SizedBox(height: 10),
              pw.Text('Gracias por usar Rappi Team',
...
class RevenueChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final fillPaint = Paint()
      ...
      ..shader = LinearGradient(
        colors: [
          ...
          ModernTheme.rappiOrange.withValues(alpha: 0.0),
        ],
    ...
    // Draw line
    paint.color = ModernTheme.rappiOrange;
