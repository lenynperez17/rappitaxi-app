...
class MapPickerScreen extends StatefulWidget {
  ...
  const MapPickerScreen({super.key, this.initialLocation, this.title});
...
class _MapPickerScreenState extends State<MapPickerScreen> {
  ...
  Future<void> _generateRefDots(LatLng center) async {
    ...
    for (int i = 0; i < dotCount; i++) {
      ...
      final pos = LatLng(center.latitude + cos(angle) * dist, center.longitude + sin(angle) * dist);
      _markers.add(Marker(markerId: MarkerId('ref_dot_$i'), position: pos, icon: icon, anchor: const Offset(0.5, 0.5), zIndexInt: 1));
    }
    if (mounted) setState(() {});
  }
  ...
  Future<void> _getAddressFromCoordinates(LatLng location) async {
    ...
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        final parts = [p.street, p.subLocality, p.locality].where((c) => c != null && c.isNotEmpty);
        _selectedAddress = parts.isNotEmpty ? parts.join(', ') : '${location.latitude.toStringAsFixed(6)}, ${location.longitude.toStringAsFixed(6)}';
      } else {
  ...
  void _confirmSelection() {
    Navigator.of(context).pop({
      ...
      'coordinates': {'lat': _mapCenter.latitude, 'lng': _mapCenter.longitude},
    });
  }
    ...
    return Scaffold(
      body: Stack(children: [
        GoogleMap(
          ...
          onCameraIdle: () { _getAddressFromCoordinates(_mapCenter); },
          markers: _markers, myLocationEnabled: true, myLocationButtonEnabled: false, zoomControlsEnabled: false, mapToolbarEnabled: false,
        ),
        Center(child: Padding(padding: const EdgeInsets.only(bottom: 36), child: Icon(Icons.location_on, size: 48, color: Colors.blue.shade700))),
          ...
          Center(child: Padding(
            ...
            child: Container(
              ...
              child: _isGettingAddress
                  ...
                  : Text(_selectedAddress!, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w500), maxLines: 1, overflow: TextOverflow.ellipsis),
            ),
          )),
        Positioned(
          ...
          child: GestureDetector(
            ...
            child: Container(
              width: 44, height: 44,
              decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.15), blurRadius: 8, offset: const Offset(0, 2))]),
