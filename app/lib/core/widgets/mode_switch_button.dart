...
class ModeSwitchButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<AuthProvider>(
      builder: (context, authProvider, _) {
        ...
        // Admins con múltiples roles SÍ pueden ver el botón de cambio
        if ((user.isAdmin || user.userType == 'admin') &&
            (user.availableRoles == null || user.availableRoles!.length <= 1)) {
          return SizedBox.shrink();
        }
        ...
        // Usar modo EFECTIVO considerando documentVerified
  ...
  /// Obtener modo efectivo considerando el estado de documentos
  /// Si está en modo driver pero no tiene documentos verificados,
  ...
  Future<void> _switchToMode(
    ...
    try {
      // Detener listeners del rol anterior ANTES de cambiar
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              SizedBox(width: 12),
              Expanded(
                child: Text(FirestoreErrorHandler.getSpanishMessage(e)),
              ),
            ],
          ),
          backgroundColor: RtColors.error,
