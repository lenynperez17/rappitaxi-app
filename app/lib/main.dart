...
// Eliminados: ModernRegisterScreen, ForgotPasswordScreen, EmailVerificationScreen
// Las pantallas de registro separado, recuperar contrasena y verificacion email
// ya no se usan en el flujo simplificado de autenticacion (solo Google + Telefono).
...
void main() async {
  ...
  AppLogger.separator('INICIANDO RAPITEAM APP');
  AppLogger.info('Iniciando aplicación RapiTeam...');
  ...
  try {
    ...
    // Solo inicializar Firebase si NO esta ya inicializado.
    // En iOS, AppDelegate.swift llama FirebaseApp.configure() ANTES
    // de que este codigo ejecute. Si intentamos inicializar de nuevo,
    ...
    runApp(RapiteamApp(preferencesProvider: preferencesProvider));

  } catch (error, stackTrace) {
    AppLogger.error('Error crítico al inicializar', error, stackTrace);
    final fallbackProvider = PreferencesProvider();
    runApp(RapiteamApp(preferencesProvider: fallbackProvider));
  }
}

class RapiteamApp extends StatelessWidget {
  final PreferencesProvider preferencesProvider;

  const RapiteamApp({super.key, required this.preferencesProvider});
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      ...
      child: NotificationHandlerWidget(
        // Dismiss global del teclado - funciona en toda la aplicacion
        child: Builder(
...
// Usar Consumer para garantizar rebuild cuando cambia darkMode
class _ThemedMaterialApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<PreferencesProvider>(
      ...
      // Tema del design system RapiTeam
      theme: RtTheme.light,
      darkTheme: RtTheme.dark,
      themeMode: themeMode,
      ...
      routes: {
        ...
        // Rutas eliminadas: /register, /forgot-password, /email-verification
        // El flujo simplificado usa solo Google + Telefono, sin registro separado.
