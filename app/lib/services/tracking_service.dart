      ...
      if (response.statusCode == 200) {
        ...
        if (data['success']) {
          ...
          return RouteResult.success(
            distance: (routeData['distance'] as num?)?.toDouble() ?? 0.0, // metros
            duration: (routeData['duration'] as num?)?.toInt() ?? 0, // segundos
            polyline: routeData['polyline'],
            steps: (routeData['steps'] as List).map((step) => RouteStep(
              instruction: step['instruction'],
              distance: (step['distance'] as num?)?.toDouble() ?? 0.0,
              duration: (step['duration'] as num?)?.toInt() ?? 0,
              startLocation: LatLng(
                (step['startLocation']['latitude'] as num?)?.toDouble() ?? 0.0,
                (step['startLocation']['longitude'] as num?)?.toDouble() ?? 0.0,
              ),
              endLocation: LatLng(
                (step['endLocation']['latitude'] as num?)?.toDouble() ?? 0.0,
                (step['endLocation']['longitude'] as num?)?.toDouble() ?? 0.0,
              ),
            )).toList(),
          );
        } else {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success']) {
          ...
          return ETAResult.success(
            ...
            duration: (etaData['duration'] as num?)?.toInt() ?? 0,
            distance: (etaData['distance'] as num?)?.toDouble() ?? 0.0,
          );
        } else {
        ...
        if (data['success']) {
          ...
          return TrackingInfo(
            ...
            currentLocation: LatLng(
              (trackingData['currentLocation']['latitude'] as num?)?.toDouble() ?? 0.0,
              (trackingData['currentLocation']['longitude'] as num?)?.toDouble() ?? 0.0,
            ),
            destination: LatLng(
              (trackingData['destination']['latitude'] as num?)?.toDouble() ?? 0.0,
              (trackingData['destination']['longitude'] as num?)?.toDouble() ?? 0.0,
            ),
            estimatedArrival: DateTime.parse(trackingData['estimatedArrival']),
            totalDistance: (trackingData['totalDistance'] as num?)?.toDouble() ?? 0.0,
            totalDuration: (trackingData['totalDuration'] as num?)?.toInt() ?? 0,
          ...
          if (data['rideId'] == rideId) {
            final update = TrackingUpdate(
              ...
              currentLocation: data['currentLocation'] != null
                ? LatLng(
                    (data['currentLocation']['latitude'] as num?)?.toDouble() ?? 0.0,
                    (data['currentLocation']['longitude'] as num?)?.toDouble() ?? 0.0,
                  )
                : null,
              ...
              deviationDistance: (data['deviation']?['deviationDistance'] as num?)?.toDouble(),
