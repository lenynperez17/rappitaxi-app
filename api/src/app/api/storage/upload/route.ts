...
import {
  saveBuffer, sniffMime, isMimeAllowed,
  type StorageScope,
} from '@/lib/storage'
...
export async function POST(req: NextRequest) {
  ...
  const sniffedMime = sniffMime(buf) ?? file.type
  if (!isMimeAllowed(sniffedMime)) {
    return NextResponse.json({
  ...
  try {
    const { key, sha256, size } = await saveBuffer(scope, auth.userId, buf, sniffedMime)
    ...
    const rows = await query<{ id: string }>(
      `INSERT INTO storage_files (user_id, scope, storage_key, mime, size_bytes, sha256_hex, is_public)
       ...
       RETURNING id`,
      [auth.userId, scope, key, sniffedMime, size, sha256, isPublic],
    )
