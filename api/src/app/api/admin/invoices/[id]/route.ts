...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const current = await maybeOne<{ status: string }>(`SELECT status FROM invoices WHERE id = $1`, [id])
  ...
  if (body.status === 'voided') {
    ...
    // No permitir anular directamente si ya tiene notas de crédito emitidas —
    // el circuito correcto es emitir una nota de anulación desde /credit-notes.
    const hasNotes = await maybeOne<{ count: string }>(
      `SELECT COUNT(*)::text AS count FROM credit_notes WHERE invoice_id = $1`,
      [id],
    )
    if (Number(hasNotes?.count ?? '0') > 0) {
    ...
    const params: unknown[] = body.notes ? [id, auth.userId, body.notes] : [id, auth.userId]
    const updated = await maybeOne<Row>(
      `UPDATE invoices
          SET status = 'voided',
              voided_at = NOW(),
