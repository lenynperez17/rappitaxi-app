...
export class MercadoPagoService {
  ...
  async processWithdrawal(params: {
    ...
    try {
      ...
      const transferData = {
        ...
        description: `Retiro Rappi Team - ${params.driverId}`,
  ...
  async processCheckoutBricksPayment(params: {
    ...
    try {
      ...
      if (params.payerFirstName) payer.first_name = params.payerFirstName;
      if (params.payerLastName) payer.last_name = params.payerLastName;
  ...
  private async handleRechargePayment(rechargeId: string, payment: any): Promise<void> {
    try {
      ...
      if (payment.status === 'approved') {
        ...
        // También actualizar en users para consistencia
